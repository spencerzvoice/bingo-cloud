...
- **2026-09-24 — Follow-up engine (Spencer): he falls behind on follow-ups, so drafts + reminders are automatic.** Skill `followup-drafts`; scheduled tasks `followup-prep-evening` (Sun-Thu 18:00: drafts for next business day) and `followup-remind-morning` (weekdays ~09:00: 'due today, send now'). Drafts land in Gmail Drafts as threaded replies, tracked in `memory/followups-queue.md`. Never sends. The digest over-flags — always triage against live Gmail first. Runs only while the Claude desktop app is open.
