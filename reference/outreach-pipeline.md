...
6. **Found + fixed 2026-09-26 — a typo'd stored email address makes the routine's "Gmail-verified" corrections invent false negatives.** Becca Winkler's row (Xpedition Media) had `rebecca.winkler@xpedition.co` in col D (missing the "m" — real domain is `xpedition.com`). The 09/26 daily routine's Step 3 queried Gmail using that broken address, got zero hits, and instead of flagging "no activity found for this address — check it," it confidently overwrote a correct note with a false one: "no OOO auto-reply or any Gmail activity found... the previously noted claim is not supported by Gmail." Live Gmail (checked this session) showed the opposite — a real reply Jul 31 and an active OOO auto-reply from Sep 18 (back Sep 28), all under the `.com` address. Same run also wrote a stray sentence about Becca Winkler/Xpedition into Ryan Lago's (Shadow Lion) Notes — same cross-contamination pattern as hazard #3. Both rows corrected 09-26 (row 39, row 72). **Takeaway: a routine "verified" claim of absence is only as good as the stored email address — spot-check col D against the live thread, not just trust a zero-result query.**
7. **NEVER put Voices.com / Bodalgo / Voice123 marketplace work in the tracker** — not auditions, not private invites, not even a booked-and-paid job. No exceptions, no manual carve-outs. Spencer manages marketplace work on the platform itself. **Delivered marketplace jobs belong in `vo-client-directory.md` + the invoice log, NOT here.** (History: a "he may still add Zulubot Bravo Inc manually" carve-out here + a "recurring client" line in MEMORY.md let the daily routine re-add Zulubot every morning from the job-award emails in his personal Gmail. Deleted for good 2026-09-02; routine Step 4 now hard-blocks it.)
